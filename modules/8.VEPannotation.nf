process annotVEP {
    ...
    when: ! params.skip_joint

    publishDir "${params.outdir}/fastqc/Annotation", mode: 'copy'
    cpus 80
    ...
    val(params.LoFtool)
    path(params.CADDsnv)
    path(params.CADDindel)
    val(params.dbscSNV)
    val(params.assembly)
    val(params.dosage_collins)
    val(params.spliceaiindel)
    val(params.spliceaisnv)
    val(params.pli)
    val(params.phenotypesfile)
    val(params.loeuf)
    val(params.clinpred)
    val(params.alphamissense)
    val(params.primateai)
    val(params.revel)
    val(params.clinvar)

    output:
    tuple val(family_id), path("${family_id}_filtered_PASS_VEP.txt"), emit: annotVEP_vcfs

    script:
    """
    /usr/src/app/ensembl-vep/./vep --af --appris --biotype --buffer_size 40000 --cache --check_existing --offline \
        ...
        --fasta ${params.ref} \
        --force --fork 75 --mane \
        --input_file ${vcf_file} --output_file ${family_id}_filtered_PASS_VEP.txt --tab --force_overwrite \
        --polyphen b --pubmed \
